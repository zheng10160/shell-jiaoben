...
#本地参数
TAGS_PATH=/Applications/senseplay/av  ##本地地址
ENV=""
TAG=""
BUSINESS=""
TOOL="root"
...
#REMOTE_ACCOUNT="root"
#REMOTE_PATH="/data/www/av"
#HTTP_SERVER_ACCOUNT="www.senseplay.cn"
REMOTE_IP="192.168.99.62"
REMOTE_ACCOUNT="root"
REMOTE_PATH="/home/www/av"
HTTP_SERVER_ACCOUNT="www.senseplay.cn"
...
usage()
{
	...
	echo "usage: -e <dev|pro> -b <domain1|domain2> -v <v0.1> -p <file://..> -t <shell>";
	echo "tip :: $1";
	exit 1;
}
...
{
	case "$BUSINESS" in
		torrent) REMOTE_PATH=$REMOTE_PATH;;
...
do_deploy()
{
	...
	case "$TOOL" in
		...
		*) usage "Please use svn or git to deploy";;
		# shell）
               rsync -vrtL --progress $TAGS_PATH/* root@${REMOTE_IP}:${REMOTE_PATH};
               ;;
	esac;
	cd $NOW_PATH
